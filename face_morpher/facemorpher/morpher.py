...
::
  ...
  Morph through all work in a folder

  Usage:
    morpher.py (--src=<src_path> --dest=<dest_path> | --work=<folder>)
  ...
  Options:
    ...
    --work=<folder>       Folderpath to work
    --width=<width>         Custom width of the work/video [default: 500]
    --height=<height>       Custom height of the work/video [default: 600]
    --num=<num_frames>      Number of morph frames [default: 20]
    --fps=<fps>             Number frames per second for the video [default: 10]
    --out_frames=<folder>   Folder path to save all image frames
    --out_video=<filename>  Filename to save a video
    --plot                  Flag to plot work to result.png [default: False]
    --background=<bg>       Background of work to be one of (black|transparent|average) [default: black]
...
def verify_args(args):
  if args['--work'] is None:
    ...
    valid = os.path.isdir(args['--work'])
    if not valid:
      print('--work=%s is not a valid directory' % args['--work'])
      exit(1)
...
def morpher(imgpaths, width=500, height=600, num_frames=20, fps=10,
            out_frames=None, out_video=None, plot=False, background='black'):
  """
  Create a morph sequence from multiple work in imgpaths
...
def main():
  ...
  morpher(list_imgpaths(args['--work'], args['--src'], args['--dest']),
